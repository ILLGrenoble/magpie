...
if not use_pointwise:
	for twin_angle in twin_angles:
		hstart, kstart, lstart = mag.RotateQ(hkl_start[0], hkl_start[1], hkl_start[2],
			twin_axis[0], twin_axis[1], twin_axis[2], twin_angle)
		hend, kend, lend = mag.RotateQ(hkl_end[0], hkl_end[1], hkl_end[2],
			twin_axis[0], twin_axis[1], twin_axis[2], twin_angle)

		data_disp = mag.CalcDispersion(
			hstart, kstart, lstart,
			hend, kend, lend,
	...
	if use_threadpool:
		...
		with executor(max_workers = max_threads) as exe:
			...
			for hkl in numpy.linspace(hkl_start, hkl_end, num_Q_points):
				for twin_angle in twin_angles:
					h, k, l = mag.RotateQ(hkl[0], hkl[1], hkl[2],
						twin_axis[0], twin_axis[1], twin_axis[2], twin_angle)

					Es_futures.append(
						exe.submit(calc_Es, h, k, l))
		...
		for hkl in numpy.linspace(hkl_start, hkl_end, num_Q_points):
			for twin_angle in twin_angles:
				...
				for S in mag.CalcEnergies(h, k, l, False):
					...
					append_data(h, k, l, S.E, S.weight_perp)

					if print_dispersion:
						print("{:15.4f} {:15.4f} {:15.4f} {:15.4f} {:15.4g}".format(
							h, k, l, S.E, S.weight_perp))
# -----------------------------------------------------------------------------
